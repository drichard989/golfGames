...
(() => {
    ...
    const ids = {
        ...
        // Games toggles
        toggleVegas: "#toggleVegas",
        toggleBanker: "#toggleBanker",
        toggleSkins: "#toggleSkins",
        vegasSection: "#vegasSection",
        ...
        // Banker
        bankerPointValue: "#bankerPointValue",
        bankerRotation: "#bankerRotation",
        bankerUseNet: "#bankerUseNet",
        bankerDoubleBirdie: "#bankerDoubleBirdie",
        bankerTripleEagle: "#bankerTripleEagle",
        bankerBody: "#bankerBody",
        bankerTotP1: "#bankerTotP1",
        bankerTotP2: "#bankerTotP2",
        bankerTotP3: "#bankerTotP3",
        bankerTotP4: "#bankerTotP4",
        ...
        skinsHalf: "#skinsHalf",
        skinsBody: "#skinsBody",
        skinsPotTot: "#skinsPotTot",
        skinsTotP1: "#skinsTotP1",
        skinsTotP2: "#skinsTotP2",
        skinsTotP3: "#skinsTotP3",
        skinsTotP4: "#skinsTotP4",
        skinsP1: "#skinsP1",
        skinsP2: "#skinsP2",
        skinsP3: "#skinsP3",
        skinsP4: "#skinsP4",
    ...
    function buildPlayerRows() {
        ...
        for (let p = 0; p < PLAYERS; p++) {
            ...
            chInput.addEventListener("input", () => {
                if (chInput.value !== "") chInput.value = clampInt(chInput.value, -50, 60);
                recalcAll();
                vegas_recalc();
                banker_recalc();
                saveDebounced();
            ...
            for (let h = 1; h <= HOLES; h++) {
                ...
                inp.addEventListener("input", () => {
                    if (inp.value !== "") {
                        const v = clampInt(inp.value, 1, 20);
                        if (String(v) !== inp.value) inp.classList.add("invalid");
    ...
    function getNetNDB(playerIdx, holeIdx) {
        const adjCH = adjustedCHs()[playerIdx],
            gross = getGross(playerIdx, holeIdx);
        if (!gross) return 0;
        const sr = strokesOnHole(adjCH, holeIdx),
            ndb = PARS[holeIdx] + 2 + sr,
    ...
    function recalcRow(rowEl) {
        ...
        const parTotal = sum(PARS),
            delta = total && parTotal ? total - parTotal : 0,
            el = $(".to-par", rowEl);
        if (!total) {
            el.textContent = "—";
            el.dataset.sign = "";
    ...
    function recalcTotalsRow() {
        for (let h = 1; h <= HOLES; h++) {
            const ph = $$(`input.score-input[data-hole="${h}"]`).map(i => Number(i.value) || 0),
                t = sum(ph);
            $(`[data-hole-total="${h}"]`).textContent = t ? String(t) : "—";
        }
        const tds = $(ids.totalsRow).querySelectorAll("td"),
            base = LEADING_FIXED_COLS + HOLES;
    ...
    function saveState() {
        const state = {
            courseName: $(ids.courseName)?.value || "",
            teeName: $(ids.teeName)?.value || "",
    ...
    function loadState() {
        ...
        try {
            ...
            $(ids.courseName).value = s.courseName || "";
            $(ids.teeName).value = s.teeName || "";
    ...
    function games_open(which) {
        if (which === "vegas") {
            ...
            $(ids.vegasSection).setAttribute("aria-hidden", "false");
            $(ids.toggleVegas).classList.add("active");
        }
        if (which === "banker") {
    ...
    function games_close(which) {
        if (which === "vegas") {
            $(ids.vegasSection).classList.remove("open");
            $(ids.vegasSection).setAttribute("aria-hidden", "true");
            $(ids.toggleVegas).classList.remove("active");
        }
        if (which === "banker") {
            $(ids.bankerSection).classList.remove("open");
            $(ids.bankerSection).setAttribute("aria-hidden", "true");
            $(ids.toggleBanker).classList.remove("active");
        }
    }
    ...
    function vegas_renderTeamControls() {
        ...
        for (let i = 0; i < PLAYERS; i++) {
            ...
            aWrap.appendChild(a);
            aWrap.appendChild(document.createTextNode("Team A"));
            const bWrap = document.createElement("label");
            bWrap.className = "radio";
            ...
            bWrap.appendChild(b);
            bWrap.appendChild(document.createTextNode("Team B"));
            row.append(label, aWrap, bWrap);
            box.appendChild(row);
        }
    ...
    function vegas_setTeamAssignments(t) {
        for (let i = 0; i < PLAYERS; i++) {
            const a = $(`input[name="vegasTeam_${i}"][value="A"]`),
                b = $(`input[name="vegasTeam_${i}"][value="B"]`);
            if (!a || !b) continue;
            a.checked = false;
            b.checked = false;
        }
    ...
    function vegas_getOptions() {
        return {
            useNet: $(ids.optUseNet)?.checked || false,
    ...
    function vegas_setOptions(o) {
        if ('useNet' in o) $(ids.optUseNet).checked = !!o.useNet;
    ...
    function vegas_pairToString(pair) {
    ...
    function vegas_multiplierForWinner(winnerPlayers, h, useNet, opts) {
        const {
            birdie,
            eagle
        } = vegas_teamHasBirdieOrEagle(winnerPlayers, h, useNet);
        let m = 1;
    ...
    function vegas_recalc() {
        ...
        if (!(teams.A.length === 2 && teams.B.length === 2)) {
            ...
            $(ids.vegasTotalA).textContent = "—";
            $(ids.vegasTotalB).textContent = "—";
            $(ids.vegasPtsA).textContent = "—";
            return;
        }
        warn.hidden = true;
        ...
        for (let h = 0; h < HOLES; h++) {
            ...
            const vaStr = vegas_pairToString(effA),
                vbStr = vegas_pairToString(effB);
            const va = Number(vaStr),
                vb = Number(vbStr);
    ...
    function rowToPlayerObj(headerMap, row) {
        ...
        const obj = {
            player: get("player") || "",
            ch: get("ch") != null && get("ch") !== "" ? Number(get("ch")) : "",
            holes: []
        };
        for (let i = 1; i <= 18; i++) {
            const key = `h${i}`;
    ...
    async function handleCSVFile(file) {
        ...
        const required = ["player", "ch", ...Array.from({
            length: 18
        }, (_, i) => `h${i+1}`)];
        ...
        if (!rows.length) {
            alert("No data rows found under the header.");
            return;
        }
        ...
        rows.forEach((r, idx) => {
            ...
            const rowEl = playerRows[idx];
            if (!rowEl) return;
            const nameInput = rowEl.querySelector(".name-edit");
            nameInput.value = obj.player || `Player ${idx+1}`;
            const chInput = rowEl.querySelector(".ch-input");
            chInput.value = (obj.ch === 0 || Number.isFinite(obj.ch)) ? String(obj.ch) : "";
        ...
        for (let i = rows.length; i < 4; i++) {
            const rowEl = playerRows[i];
            if (!rowEl) continue;
            rowEl.querySelector(".name-edit").value = "";
            rowEl.querySelector(".ch-input").value = "";
            rowEl.querySelectorAll("input.score-input").forEach(inp => {
                inp.value = "";
                inp.classList.remove("invalid");
            });
        }
    ...
    function downloadCSVTemplate() {
        const headers = ["player", "ch", ...Array.from({
            length: 18
        }, (_, i) => `h${i+1}`)];
        const rows = [
    ...
    function init() {
        ...
        vegas_renderTeamControls();
        vegas_renderTable();
        $(ids.optUseNet).addEventListener("change", () => {
            vegas_recalc();
            saveDebounced();
        });
        $(ids.optDoubleBirdie).addEventListener("change", () => {
            vegas_recalc();
            saveDebounced();
        });
        $(ids.optTripleEagle).addEventListener("change", () => {
            vegas_recalc();
            saveDebounced();
        });
        ...
        // Banker UI + wiring
        banker_renderTable();
        $(ids.bankerPointValue).addEventListener("input", () => {
            banker_recalc();
            saveDebounced();
        });
        $(ids.bankerRotation).addEventListener("change", () => {
            banker_recalc();
            saveDebounced();
        });
        $(ids.bankerUseNet).addEventListener("change", () => {
            banker_recalc();
            saveDebounced();
        });
        $(ids.bankerDoubleBirdie).addEventListener("change", () => {
            banker_recalc();
            saveDebounced();
        });
        $(ids.bankerTripleEagle).addEventListener("change", () => {
            banker_recalc();
            saveDebounced();
        });
    ...
    function vegas_renderTable() {
        const body = $(ids.vegasTableBody);
        body.innerHTML = "";
        for (let h = 0; h < HOLES; h++) {
            const tr = document.createElement("tr");
            tr.innerHTML = `<td>${h+1}</td><td data-vegas-a="${h}">—</td><td data-vegas-b="${h}">—</td><td data-vegas-m="${h}">—</td><td data-vegas-p="${h}">—</td>`;
            body.appendChild(tr);
        }
    }
    ...
    function strokesOnHoleHalfAware(adjCH, i, half) {
        ...
        const base = Math.floor(useAdj / 18),
            rem = useAdj % 18,
            holeHcp = HCPMEN[i];
        return base + (holeHcp <= rem ? 1 : 0);
    }
    ...
    document.getElementById('toggleSkins')?.addEventListener('click', () => {
        const sec = document.getElementById('skinsSection');
        const open = !sec.classList.contains('open');
        sec.classList.toggle('open', open);
        sec.setAttribute('aria-hidden', open ? 'false' : 'true');
        document.getElementById('toggleSkins')?.classList.toggle('active', open);
        if (open) {
            setTimeout(() => {
                initSkins();
            }, 0);
