/* ============================================================================
   ...
   Dynamic scorecard supporting 1-99 players with multiple side games:
   - Course Handicap input per player (supports negative handicaps)
   - Net scoring with NDB cap; strokes allocated via "play-off-low" system
   - Vegas: team game with multipliers and birdie/eagle bonuses
   - Banker: points-per-match with rotation or until-beaten modes
   - Skins: net/gross options with carry-over and half-pops
   - Junk (Dots): points for eagles/birdies/pars with optional NET scoring
   ...
   This file is organized into clear modules (object namespaces):
   
   📦 Config         - Course database, constants, active course state
   📦 Utils          - DOM helpers ($, $$), math utilities (sum, clamp)
   📦 Storage        - localStorage save/load with versioning
   📦 Scorecard      - Table building, calculations, player/course management
      ├─ build       - buildHeader, buildParAndHcpRows, buildPlayerRows, buildTotalsRow
      ├─ calc        - adjustedCHs, strokesOnHole, recalcRow, recalcAll
      ├─ player      - syncOverlay, updateCountDisplay
      └─ course      - switch, updateParAndHcpRows, updateParBadge
   📦 Games          - Side game orchestration and state management
   📦 Vegas          - Vegas game module (compute + render)
   📦 Junk           - Junk/Dots game module (compute + render)
   📦 Skins          - Skins game module (compute + render)  
   📦 Banker         - Banker game module (compute + render)
   📦 App            - Main initialization and event wiring
...
// CSV
  ...
  const Scorecard = {
    ...
    build: {
       ...
       * - nameInput: syncPlayerNamesOverlay, vegas_renderTeamControls, saveDebounced
       * - chInput: recalcAll, AppManager.recalcGames, saveDebounced
       * - scoreInput: recalcRow, recalcTotalsRow, AppManager.recalcGames, saveDebounced
       */
      playerRows(){
